...
namespace Greenshot.Addon.Picasa
{
	...
	public sealed class PicasaDestination : AbstractDestination
	{
		...
		[Import]
		private IPicasaConfiguration PicasaConfiguration { get; set; }

		[Import]
		private IPicasaLanguage PicasaLanguage { get; set; }
		...
		private async Task<INotification> ExportCaptureAsync(ICapture capture, CancellationToken token = default(CancellationToken))
		{
			...
			try
			{
				var uploadUrl = await PleaseWaitWindow.CreateAndShowAsync(Designation, PicasaLanguage.CommunicationWait, async (progress, pleaseWaitToken) => { return await PicasaUtils.UploadToPicasa(capture, progress, token).ConfigureAwait(false); }, token);
