...
namespace Greenshot.Helpers {
	...
	public class MapiMailMessage {
		...
		private const string MAPI_LOCATION = @"SOFTWARE\Microsoft\Windows Messaging Subsystem";
		private const string MAPI_KEY = @"MAPI";
		public static bool HasMAPI() {
			using (RegistryKey key = Registry.LocalMachine.OpenSubKey(MAPI_LOCATION, false)) {
				if (key != null) {
		...
		public static void SendImage(Image image, ICaptureDetails captureDetails) {
			...
			try {
				ImageOutput.Save(image, tmpFile, captureDetails);
			} catch (Exception e) {
				// Show the problem
