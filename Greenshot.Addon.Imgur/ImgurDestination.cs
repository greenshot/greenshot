...
namespace Greenshot.Addon.Imgur
{
	...
	public sealed class ImgurDestination : AbstractDestination
	{
		...
		[Import]
		private IImgurConfiguration ImgurConfiguration { get; set; }

		[Import]
		private IImgurLanguage ImgurLanguage { get; set; }
		...
		private async Task<INotification> ExportCaptureAsync(ICapture capture, CancellationToken token = default(CancellationToken))
		{
			...
			try
			{
				...
				var imgurInfo = await PleaseWaitWindow.CreateAndShowAsync(Designation, ImgurLanguage.CommunicationWait, async (progress, pleaseWaitToken) => { return await ImgurUtils.UploadToImgurAsync(_oauth2Settings, capture, outputSettings, capture.CaptureDetails.Title, filename, progress, pleaseWaitToken); }, token);
