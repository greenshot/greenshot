...
namespace GreenshotPlugin.IniFile
{
	public class IniConfig
	{
		...
		public static IEnumerable<string> IniSectionNames
		{
			get
			{
				foreach (var sectionName in SectionMap.Keys)
				{
		...
		public static void ForceIniInStartupPath()
		{
			...
			var applicationStartupPath = Path.GetDirectoryName(Assembly.GetEntryAssembly().Location);
			...
			var forcedIni = Path.Combine(applicationStartupPath, _applicationName + IniExtension);
		...
		{
			var assemblyProductAttributes = Assembly.GetEntryAssembly().GetCustomAttributes(typeof(AssemblyProductAttribute), false) as AssemblyProductAttribute[];
		...
		private static string CreateIniLocation(string configFilename, bool isReadOnly)
		{
			...
			if (applicationStartupPath != null)
			{
				var pafPath = Path.Combine(applicationStartupPath, @"App\" + _applicationName);
				...
				if (IsPortable || !_portableCheckMade)
				{
					...
					if (IsPortable)
					{
						var pafConfigPath = Path.Combine(applicationStartupPath, @"Data\Settings");
						try
						{
			...
			if (iniFilePath == null)
			{
				...
				if (!File.Exists(iniFilePath))
				{
					var iniDirectory = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), _applicationName);
		...
		private static void FixProperties(IniSection section)
		{
			...
			foreach (var fixedPropertyKey in fixedPropertiesForSection.Keys)
			{
		...
		private static IDictionary<string, IDictionary<string, string>> Read(string iniLocation)
		{
			...
			foreach (var section in newSections.Keys)
			{
				...
				else
				{
					...
					{
						var propertyValue = newProperties[propertyName];
		...
		{
			var sectionName = section.IniSectionAttribute.Name;
		...
		public static void Save()
		{
			...
			try
			{
				...
				if (acquiredLock)
				{
					...
					var iniLocation = CreateIniLocation(_configName + IniExtension, false);
					try
					{
		...
		private static void SaveInternally(string iniLocation)
		{
			...
			using (var memoryStream = new MemoryStream())
			{
				using (TextWriter writer = new StreamWriter(memoryStream, Encoding.UTF8))
				{
					...
					using (var fileStream = new FileStream(iniLocation, FileMode.Create, FileAccess.Write))
					{
