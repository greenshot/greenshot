...
namespace GreenshotPlugin.Core
{
	...
	public class CoreConfiguration : IniSection, INotifyPropertyChanged
	{
		...
		[IniProperty("OutputPrintAllowRotate", LanguageKey = "printoptions_allowrotate", Description = "Allow rotating the picture for fitting on paper?",
			DefaultValue = "false")]
		public bool OutputPrintAllowRotate { get; set; }

		[IniProperty("OutputPrintAllowEnlarge", LanguageKey = "printoptions_allowenlarge", Description = "Allow growing the picture for fitting on paper?",
			DefaultValue = "false")]
		public bool OutputPrintAllowEnlarge { get; set; }

		[IniProperty("OutputPrintAllowShrink", LanguageKey = "printoptions_allowshrink", Description = "Allow shrinking the picture for fitting on paper?",
			DefaultValue = "true")]
		...
		[IniProperty("ExperimentalFeatures", Description = "A list of experimental features, this allows us to test certain features before releasing them.",
			ExcludeIfNull = true)]
		...
		public override void AfterLoad()
		{
			...
			if (NoGDICaptureForProduct != null)
			{
				...
				for (var i = 0; i < NoGDICaptureForProduct.Count; i++)
				{
			...
			if (NoDWMCaptureForProduct != null)
			{
				...
				for (var i = 0; i < NoDWMCaptureForProduct.Count; i++)
				{
