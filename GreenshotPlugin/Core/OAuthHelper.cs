...
namespace GreenshotPlugin.Core
{
	...
	public class OAuth2Settings
	{
		...
		/// <summary>
		///     Expire time for the AccessToken, this this time (-60 seconds) is passed a new AccessToken needs to be generated
		...
		/// <summary>
		///     Put anything in here which is needed for the OAuth 2 implementation of this specific service but isn't generic,
		///     e.g. for Google there is a "scope"
		/// </summary>
	...
	public class OAuthSession
	{
		...
		///     "Private" key for the encoding. when usin RSASHA1 this is the password for the private key
		///     file
		/// </param>
		...
		public static string UrlEncode3986(string value)
		{
			...
			foreach (var symbol in value)
			{
				...
				{
					var utf8Bytes = Encoding.UTF8.GetBytes(symbol.ToString());
		...
		private string GetRequestToken()
		{
			...
			var response = MakeRequest(RequestTokenMethod, RequestTokenUrl, null, parameters, null);
		...
		/// </summary>
		/// <param name="requestTokenResponse">
		///     Pass the response from the server's request token, so if there is something wrong we
		///     can show it.
		/// </param>
		...
		private string GetAccessToken()
		{
			...
			var response = MakeRequest(AccessTokenMethod, AccessTokenUrl, null, parameters, null);
			if (!string.IsNullOrEmpty(response))
			{
				response = NetworkHelper.UrlDecode(response);
			...
			IDictionary<string, object> additionalParameters, IBinaryContainer postData)
		{
			...
			while (retries-- > 0)
			{
				...
				catch (UnauthorizedAccessException uaEx)
				{
					...
					foreach (var parameterKey in parametersToSign.Keys)
					{
		...
		private void Sign(HTTPMethod method, string requestUrl, IDictionary<string, object> parameters)
		{
			...
			var normalizedUrl = string.Format(CultureInfo.InvariantCulture, "{0}://{1}", url.Scheme, url.Host);
		...
		/// <summary>
		///     Make the actual OAuth request, all oauth parameters are passed as header (default) and the others are placed in the
		///     url or post data.
		...
		private string MakeRequest(HTTPMethod method, string requestUrl, IDictionary<string, string> headers, IDictionary<string, object> parameters, IBinaryContainer postData)
		{
			...
			// Create webrequest
			var webRequest = NetworkHelper.CreateWebRequest(requestUrl, method);
			...
			if ((HTTPMethod.POST == method || HTTPMethod.PUT == method) && postData == null && requestParameters.Count > 0)
			{
				...
				else
				{
					...
					foreach (var parameterKey in requestParameters.Keys)
					{
					...
					var data = Encoding.UTF8.GetBytes(form.ToString());
...
<script type='text/javascript'>
	...
	public static class OAuth2Helper
	{
		...
		public static void GenerateRefreshToken(OAuth2Settings settings)
		{
			...
			var webRequest = NetworkHelper.CreateWebRequest(settings.TokenUrl, HTTPMethod.POST);
			NetworkHelper.UploadFormUrlEncoded(webRequest, data);
			var accessTokenJsonResult = NetworkHelper.GetResponseAsString(webRequest, true);

			var refreshTokenResult = JSONHelper.JsonDecode(accessTokenJsonResult);
			...
			var seconds = refreshTokenResult[ExpiresIn];
			if (seconds != null)
			{
		...
		public static void GenerateAccessToken(OAuth2Settings settings)
		{
			...
			var accessTokenResult = JSONHelper.JsonDecode(accessTokenJsonResult);
			...
			var seconds = accessTokenResult[ExpiresIn];
			if (seconds != null)
			{
				settings.AccessTokenExpires = DateTimeOffset.Now.AddSeconds((double) seconds);
			}
		...
		private static bool AuthenticateViaLocalServer(OAuth2Settings settings)
		{
			...
			var result = codeReceiver.ReceiveCode(settings);
		...
		public static HttpWebRequest CreateOAuth2WebRequest(HTTPMethod method, string url, OAuth2Settings settings)
		{
			...
			var webRequest = NetworkHelper.CreateWebRequest(url, method);
