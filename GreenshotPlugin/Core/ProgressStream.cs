...
namespace GreenshotPlugin.Core {
	...
	public class ProgressStream : StreamDelegate {
		...
		/// </summary>
		public Nullable<long> TotalBytesToReceive {
			get;
			set;
		}
		...
		/// </summary>
		public Nullable<long> TotalBytesToSend {
			get;
			set;
		}
       ...
       public override async Task WriteAsync(byte[] buffer, int offset, int count, CancellationToken cancellationToken)
        ...
        {
			var asyncResult = base.BeginWrite(buffer, offset, count, callback, state);
			// TODO: would be nicer if we would wait for it to end...
			ReportBytesSent(count);
            return asyncResult;
        }
