...
4.  **Serialization**: The commands to be sent are encapsulated in an `AppCommands` object, which is then placed inside an `IpcMessage` DTO. This `IpcMessage` is serialized into a byte array using JSON serialization from `System.Text.Json`.

5.  **Transmission**: The serialized byte array is written to the named pipe using a `NamedPipeClientStream`.

6.  **Receiving and Deserialization**: The `IpcListener` of the receiving instance reads the byte array from the pipe, deserializes it back into an `IpcMessage` using JSON deserialization from `System.Text.Json`, and extracts the `AppCommands`.
